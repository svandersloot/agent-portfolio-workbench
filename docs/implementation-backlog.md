...
| 8.4 | Standardize publish-history generation | P1 | Confluence publisher output | Space admin | After an approved apply, maintainer can create a safe local record with page ID, title, old version, new version, source file, command, timestamp, and verification result without tokens or private config. The publish-history-record automation itself remains manual. (Cross-reference: the monthly governance snapshot in item 5.7 consumes `docs/publish-history/` as one input; see `docs/reports/governance-report-generation-process.md`.) |
