...
| 8.4 | Standardize publish-history generation | P1 | Confluence publisher output | Space admin | After an approved apply, maintainer can create a safe local record with page ID, title, old version, new version, source file, command, timestamp, and verification result without tokens or private config. Related repeatable-reporting work is documented locally: `docs/reports/governance-report-generation-process.md` defines the smallest repeatable process for the three governance reports and names `docs/publish-history/` as a source input for the monthly snapshot; `scripts/Test-GovernanceReportInputs.ps1` is a read-only preflight that confirms the named inputs exist. The publish-history-record automation itself remains manual. |
