...
| 7.11 | Now | Most planned agents are team-facing, so they need the fuller agentic development pattern: operating contract, scoped context, guardrails, evaluation, promotion packet, and change evidence. This reduces risk before team use without making normal users learn the repo. | Make this the default for team-facing agents while allowing lightweight variants for personal, one-off, or low-risk helpers. Keep the Shark Tank story high-level: ROVO is the user-facing runtime, and the Agent Lab/repo pattern is the proposed approved workshop/control layer. |
| 7.12 | Now | The project already reflects many durable values and first-principles habits, but they are implicit and scattered. Incoming collaborators need a concise foundation that explains why truthfulness, stewardship, human accountability, scoped context, and smallest-useful-slice rules matter. | Create a repo-facing principles foundation and collaborator checklist. Frame Biblical principles in work-safe operating language such as truthfulness, stewardship, humility, service, justice/fairness, care, and accountability; use first-principles reasoning as the secondary design method. |
...
| 7.11 | Define fuller pattern for team-facing agents | P0 | 7.1, 7.3, 8.11 | Governance steward + agent owners | Team-facing agents default to a fuller package: runtime contract, Runtime Context Map, guardrails, knowledge source plan, evaluation prompt pack, Studio promotion packet, change log, owner/backup owner, measurement, and post-Studio parity check. Lightweight variants are documented for personal or low-risk helpers. Shark Tank wording avoids claiming the Agent Lab is already official work infrastructure and presents it as the proposed approved workshop/control layer. |
