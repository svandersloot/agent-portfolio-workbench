...
| 5.5 | Create governance completeness report | P1 | 1.1, 5.4 | Space admin | First read-only dry-run exists at `docs/reports/governance-completeness-dry-run.md`. Repeatable process now documented: `docs/reports/governance-report-generation-process.md` (Report 1) gives the regeneration steps and field-level output rules, and `scripts/Test-GovernanceReportInputs.ps1` is a read-only input preflight. Next: run it after inventory cleanup and normalized Studio captures are current. |
| 5.6 | Create duplicate-agent discovery report | P1 | 1.1 | Working group lead | First read-only dry-run exists at `docs/reports/duplicate-agent-discovery-dry-run.md`. Repeatable process now documented: `docs/reports/governance-report-generation-process.md` (Report 2) gives the one-group-at-a-time regeneration steps. Next: compare duplicate page bodies one page group at a time before any legacy-banner or archive action. |
| 5.7 | Create monthly governance snapshot | P1 | 5.5, 5.6 | Space admin | First snapshot exists at `docs/reports/monthly-governance-snapshot-2026-05.md`; reusable template exists at `templates/confluence/monthly-governance-snapshot-template.md`. Repeatable process now documented: `docs/reports/governance-report-generation-process.md` (Report 3) regenerates the snapshot from named inputs (inventory, completeness, publish history, prior reports), and `scripts/Test-GovernanceReportInputs.ps1` confirms those inputs exist. Next: generate future snapshots from inventory, completeness, publish history, and intake data. |
...
| 7.11 | Define fuller pattern for team-facing agents | P0 | 7.1, 7.3, 8.11 | Governance steward + agent owners | Done locally: `docs/team-facing-agent-pattern.md` sets the default fuller package (runtime contract, Runtime Context Map, guardrails, knowledge source plan, evaluation prompt pack, Studio promotion packet, change log, owner/backup owner, measurement, and post-Studio parity check), documents the lightweight variant for personal or low-risk helpers, separates runtime/setup/governance/evaluation/promotion evidence layers, and keeps pitch wording proposal-level (ROVO is the user-facing runtime; the Agent Lab/repo pattern is the proposed approved workshop/control layer). |
...
| 8.4 | Standardize publish-history generation | P1 | Confluence publisher output | Space admin | After an approved apply, maintainer can create a safe local record with page ID, title, old version, new version, source file, command, timestamp, and verification result without tokens or private config. Proposed tiny follow-up: add a local post-apply helper that turns reviewed publisher output plus operator-provided verification notes into a `docs/publish-history/` Markdown record; do not add CI, live publishing, or private config handling in that slice. (Cross-reference: the monthly governance snapshot in item 5.7 consumes `docs/publish-history/` as one input; see `docs/reports/governance-report-generation-process.md`.) |
...
| 8.9 | Track Claude Code/work-backed repo readiness | P2 | Stage 5 readiness review | Governance steward | Mostly done locally: `docs/claude-code-adoption-plan.md` (Accepted 2026-06-29) covers branch strategy, PR review, secrets handling, publish workflow, evidence capture, and stakeholder communication; `.claude/settings.json` implements Claude-specific read-only settings derived from current repo truth. CI/automated checks delivered 2026-07-21 via issue #50: `repository-validation` extended (`docs/ci-minimum-checks.md`) and required under the `main-protection` ruleset. |
| 8.10 | Align completeness contract wording with field-level reporting | P2 | Completeness contract and backlog decision table | Governance steward | Done locally: the completeness contract "Dry-Run Report Shape" (`docs/agent-governance-completeness-contract.md`) and the monthly snapshot template (`templates/confluence/monthly-governance-snapshot-template.md`) now describe a present/total field count plus a missing-or-unproven field list instead of a percentage "Completeness" score, consistent with `docs/reports/governance-report-generation-process.md` and backlog decisions `4.6` and `8.10`. |
...
## Milestone 9: Cross-Platform And Scale-Out (Proposed — From 2026-07 Research)

Source: the 2026-07 research deliverable in `docs/research/` (`enterprise-agent-governance-operating-model-2026-07.md` and companions). **All rows below are proposed candidates**; the `Priority` column carries the research ROI ranking as a *suggested* order only. Final prioritization, sequencing, and any promotion/approval remain human-owned per `AGENTS.md`. Keep the Milestone 8 boundaries: no Confluence publish without a focused dry-run and approval; no Jira writes; no ROVO Studio writes; no Studio write-back; keep private config, raw captures, HARs, cookies, headers, tokens, and unsanitized exports out of Git.

Cross-references (already tracked — do not duplicate here): first team pilot → item `7.9`; CI for existing checks → item `8.9` remaining slice; monthly snapshot generation → items `5.7`/`5.5`-`5.6` and `docs/reports/governance-report-generation-process.md`; risk-tier in Design Agent output → item `4.4`.
...
| 9.2 | Add four governance fields to the completeness contract and record schema | P1 (proposed) | Completeness contract; `schemas/agent-governance-record.schema.json` | Governance steward | Proposed: add `risk tier (1-3)`, `data classification touched`, `platform + runtime identity`, and `write capability (none / confirm-gated / autonomous)` to `docs/agent-governance-completeness-contract.md` and the record schema, each anchored to its external rationale (research §4). Reporting stays field-level; no percentage scoring. |
| 9.3 | Extract a platform-neutral agent registry record (schema v2) | P1 (proposed) | 9.2 | Governance steward | Proposed: define a core ~16-field registry record with platform-specific extension blocks so one row can describe an agent on Rovo, Copilot, Claude, or a custom runtime (research §5, §9.4). Regenerate the completeness report from it. Repo-only; no live-system reads. |
| 9.4 | Make regression prompt packs executable as Rovo Evaluations CSV | P1 (proposed) | 8.5 regression packs | Agent owners | Proposed: generate a Rovo Agent Evaluations-compatible CSV (≤50 prompts) from each pilot agent's regression prompt pack so evaluation is a run, not a document (research build #2). Repo produces the CSV and records results only after a human runs the eval in Rovo; no claim of a live eval result from this repo. |
| 9.5 | Add a scheduled drift-diff routine over normalized captures | P2 (proposed) | Normalized capture flow | Governance steward | Proposed: a read-only routine that re-normalizes existing captures, diffs against the last recorded state, and appends a one-line drift-log entry per agent (research build #6). No Studio writes; capture remains a manual/human step; treat the capture adapter as disposable behind the normalized schema. |
| 9.6 | Create a Tier-1 self-service checklist | P2 (proposed) | 9.1, 9.2 | Governance steward | Proposed: a one-page checklist letting a builder ship a personal, read-only, no-sensitive-data (Tier 1) helper without steward review, with after-the-fact sampling (research §9.8). Does not grant promotion or broad visibility; those stay human-owned. |
| 9.7 | Register one non-Rovo agent to prove the registry generalizes | P2 (proposed) | 9.3 | Governance steward + agent owner | Proposed: add one real second-platform registry row (Copilot solution export or a Claude settings/skill file) plus a capture/normalize adapter for that source (research §9.4, build #9). Read-only ingest of an exported/sanitized config; no writes to any platform. |
| 9.8 | Add a reuse ledger | P2 (proposed) | Agent Fit Intake | Governance steward | Proposed: a running, low-cost record counting intake decisions routed to reuse / improve / merge, each linked to its evidence (research build #10). Feeds the leadership snapshot's "duplicates prevented" line. Docs-only. |
| 9.9 | Resolve governance doc-sprawl with a canonical doc + pointer pass | P1 (proposed) | None | Governance steward | Proposed: declare one canonical operating document and reduce the overlapping operating docs (AGENTS, START-HERE, operating model, Stage 5, team-facing pattern, principles) to thin pointers, then let the existing consistency checks police the repo's own docs (research §9.6). Docs-only. |
| 9.10 | Add source-freshness and model-version-drift checks to the review checklist | P2 (proposed) | Review cadence | Governance steward + agent owners | Proposed: add knowledge-source freshness and model-version drift (rerun the eval pack after a platform model-update announcement) to the per-agent review checklist (research §4). Docs/process-only; no live-system automation. |
