...
- The [canonical IA decision](reports/canonical-audience-and-lifecycle-ia-decision-2026-07-28.md) adopts the five-gate lifecycle *structure* but does not change this contract: version 1 (seven statuses, twelve required fields) stays effective as-is. Risk-tier requirements and any added fields (e.g. risk tier, data classification, platform/runtime identity, write capability) remain **proposed** until decisions `9.1` and `9.2` (the completeness-contract and schema changes) are accepted together.
