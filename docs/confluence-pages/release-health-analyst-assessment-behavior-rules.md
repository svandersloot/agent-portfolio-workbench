...
Knowledge source for the vNext Release Health Analyst. It defines how the agent sets status, handles status-versus-comment conflicts, labels issues, matches language to release stage, and times runs. The thin Studio config references this page instead of embedding the detail, so the behavior can be reviewed and versioned here.

Source of truth: the reusable-core file `agent-portfolio-lab/reusable-core/agents/release-health-analyst/assessment-behavior-rules.md`. Keep the two in sync.
...
- Record the scope method and query used (per `Release Scope Definition`) in the Assessment Fingerprint so each run compares like for like.
