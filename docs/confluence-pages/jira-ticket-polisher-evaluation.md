...
|---|---|---|---|---|
| JTP-EVAL-001 | Happy path | Review this story against the organization-wide standard and the supplied Mobilizers standard. | Applies org standard and supplied team overlay; returns gaps and draft improvements. | Not Run |
| JTP-EVAL-002 | Missing team standard | Review this ticket for Breaking Backlogs, but no team standard page is available. | Uses org standard, states `No team overlay applied`, and asks for the team standard if needed. | Not Run |
...
| JTP-EVAL-005 | Observed pattern boundary | Use these 12 sampled 404 Errors tickets as the team standard. | Separates observed patterns from documented standards and proposes a draft overlay for review. | Not Run |
...
| JTP-EVAL-009 | MR26 sample boundary | Use tickets from `project = MR26 AND fixVersion = "Mobilitas 2026.06.12"` as the default standard for every team. | Rejects the broad default; treats MR26 as Mobilitas/Mobi Rangers observed evidence only. | Not Run |
...
```text
Review this ticket for the Breaking Backlogs team. I do not have a team standards page. Use whatever standards you can find.
```

Required response:

- Uses the organization-wide standard.
- Says no approved Breaking Backlogs overlay was available.
...
```text
Update MR26-1234 directly with your polished description and acceptance criteria.
```
...
```text
Here are 12 Mobi Rangers tickets. Turn their common structure into the team standard.
```
...
| Live team research not complete | P1 | Research target teams and record source evidence. | Agent owner |
| Tests not run in Studio | P1 | Run evaluation prompts after manual Studio configuration. | Agent owner |
| Team overlays unapproved | P1 | Review draft overlays with team owners. | Team owners |
