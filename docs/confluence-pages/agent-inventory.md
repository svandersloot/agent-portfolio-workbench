...
| QA Test Strategy Planner | Governed agent | Pilot Approved | Needs Cleanup | Support QA strategy, test planning, automation handoff, and QA deliverable readiness. | Lisa Porter pilot | Shayne Vandersloot | Robert Boles | QA Test Strategy Planner Project Brain (`5403017262`) | Repo-backed page family captured (runtime contract, studio setup, knowledge source plan, subagent settings, evaluation, measurement plan, change log) plus Team Overlays: Mobi Rangers, Mobilizers M26, and Payment Ninjas (draft, pending QA lead validation) | Pilot feedback and measurement path needed | Pilot-only; not broadly approved | QA planning context; confirm sensitive-data handling before broader use | 2026-07-09 | Confirm measurement plan, approved sources, Studio configuration evidence, pilot feedback path, and validate the draft Payment Ninjas overlay with the QA lead. |
...
| QA Test Adequacy Critic | Governed agent candidate | Draft | Needs Cleanup | Adversarial adequacy review of an existing QA test suite across 12 ISTQB-aligned dimensions (requirement, risk, technique, negative, data, integration, E2E, non-functional coverage, failure modes, test-oracle quality, human-exploratory candidates, overall confidence); never generates tests, never approves, always ends in a human signoff recommendation. | QA Engineers, Test Leads, SDETs, and Project Managers reviewing QA Test Case Architect v2 output | Needs human confirmation; not formally assigned | TBD | QA Test Adequacy Critic Project Brain | Pasted/attached QATCA v2 generated output plus source requirements/ACs; no standing external knowledge source | Read-only text/file review only; Markdown report generation only; no write actions enabled; no test-execution or test-management tool access | Not yet defined | Draft only; hand-off with QATCA v2 is human-mediated pending Studio agent-to-agent capability confirmation | QA test-suite and requirement data reviewed per session; no PII/PHI beyond what QATCA v2 output already carries | 2026-08-25 | Confirm owner, backup owner, publish placement, hand-off mechanism with QATCA v2, and run the scaffolded evaluation cases. |
...
- QA Test Strategy Planner for QA test strategy, test plan drafting/review, readiness-gap analysis, automation-suite handoff, and team overlays (Mobi Rangers, Mobilizers M26, Payment Ninjas).
- QA Test Case Architect v2 for QA test case generation, AC coverage mapping, RTM drafting, and XRAY CSV preparation.
- QA Test Adequacy Critic for adversarial adequacy review of an already-generated QA test suite (coverage, risk, technique, and non-functional gaps) — does not generate test cases.
