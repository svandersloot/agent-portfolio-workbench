...
The two `Test Type` columns and `Application List` in the TestSuite/XRAY CSV are constrained to approved values. The agent must not invent a value that is not on the approved list; when it cannot confidently determine a value, it must flag the field for human review instead of guessing. Each field has its own value source — none of this is blanket inference: the first `Test Type` (execution mode) is fixed at `Manual` and is not inferred; the functional Test Type is SOURCED from the Jira story's test-type field, not inferred from prose (the list below is canonical validation data — the allowed set); only `Application List` is inferred from story/project context, constrained to its approved picklist, and presented to the user for confirmation before proceeding.

- **Execution-mode Test Type** (first `Test Type` column): the default and only currently approved value is `Manual` (decided 2026-07-16). `Generic` and `Cucumber` remain XRAY vocabulary but are not emitted unless a future approved source changes this contract; the separate `Automated` column remains Yes/No. Source: Jira/XRAY configuration.
