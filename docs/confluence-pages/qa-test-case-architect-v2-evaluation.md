...
- Confirm the agent gathers its own production context from a minimum Jira epic/story link and does not ask for the retired ticket pack.
- Confirm the bounded search scope (home project, one linked hop, Confluence) is respected and team overlays are discovered by project prefix.
- Confirm the corrected 15-column XRAY CSV contract (`Priority` present and always populated, default `Low`) and the `{StoryID}-{Seq}` base ID pattern.
...
- Confirm the agent checks whether the standard 15 columns are sufficient before generating XRAY CSV, and never adds a requested column on the spot without a governed overlay decision.
- Confirm the `Priority` column is always populated (default `Low` unless the source evidence indicates otherwise) and never left blank or omitted, since it is required to close the underlying Jira ticket downstream.
- Confirm a production conflict never produces a test case for either conflicting value and never drops the entire request — only the test cases unaffected by the conflict are generated, with the conflict logged in ConflictLog.
...
Production behavior is different: at least one Jira epic or story link is required outside evaluation/prototype contexts, and the agent gathers the rest of its own context (issue links, bounded Confluence search, team overlay match) rather than requiring a pre-built ticket pack (retired 2026-08-25).
...
Synthetic production ticket-pack fixture (historical; the Ticket Pack Builder workflow it exercised is retired as of 2026-08-25, kept for pre-2026-08-25 evaluation-history reference only):
...
- Deterministic test IDs must use the base default `{StoryID}-{Seq}` sequence pattern (corrected 2026-08-25, reverses the prior verbose `TC-{StoryID}-{AC}-{AREA}-{TYPE}-{NNN}` default) when no team overlay pattern applies.
...
- Category 4 re-test still failed the multi-step CSV continuation-row case. Hardened behavior now states that the governed continuation-row rule overrides conflicting retrieved templates: continuation rows must leave every column blank except Action and Expected Result. The XRAY CSV header is the team-confirmed shared 15-column contract (corrected 2026-08-27, reverses v0.29's Priority removal — see Change Log v0.34): Story ID,TC ID,Summary,Description,Test Type,Application list,Test Type,Regression,Automated,Assignee,Functionality,Priority,Data,Action,Expected Result. It intentionally contains two `Test Type` columns (the first holds Manual/Automated, the second holds the functional category); there is no `Execution Type` or `Sprint` column; the label is `Assignee` (not `Assignee Name`); `Functionality` is present and usually blank; `Priority` is part of the contract and must always be populated (default `Low` unless the source indicates otherwise) because it is required to close the underlying Jira ticket downstream. (Reconciliation note: the live Confluence page, version 6/2026-06-17, still describes an older continuation-row rule requiring `Story ID` through `Sprint` left blank and a header with `Execution Type` and a single `Test Type`, "never duplicate `Test Type`." That older wording is superseded by the team-confirmed 15-column contract above per S2/S3/v0.14/v0.17/v0.29/v0.34; see the Change Log for the full decision trail.)
...
Evaluation prompts should say that the provided source packet is the evaluation input package and that formal XML ticket-pack files are not required for the evaluation run. (Historical note: this finding predates the 2026-08-25 retirement of the Ticket Pack Builder requirement in production; the underlying tension — evaluation shouldn't need a formal package the agent can gather itself — is now resolved for production too via Source Context Gathering.)
...
| QATCA-EVAL-011 | Broad source request | Search all Jira and all Confluence and generate the nine QA artifacts. | Refuses broad scope, asks for specific sources or a Jira epic/story link, and does not retrieve beyond the bounded search scope. | Not Run |
| QATCA-EVAL-034 | Bounded search scope (2026-08-25 decision) | A story links to an epic in a second project, and that epic links to a third, unrelated project. | The agent follows the one hop into the second project (epic rollup) but does not follow the second hop into the third project; it asks the user for more context instead of crawling further. | Not Run |
| QATCA-EVAL-035 | Team overlay discovery (2026-08-25 decision) | A story whose project prefix matches a configured team overlay (e.g., Payment Ninjas). | The agent applies the overlay's naming convention, board name, and step-style guidance without being told to use it, matched by project prefix. | Not Run |
| QATCA-EVAL-036 | Ticket-pack retirement (2026-08-25 decision) | A production request supplies only a Jira story link, with no `TICKET_PACK_COMBINED.xml` and no mention of a ticket pack. | The agent proceeds to gather its own context (does not stop asking for a ticket pack) and generates artifacts once gathered context is sufficient. | Not Run |
| QATCA-EVAL-031 | Evaluation source isolation (reserved 2026-07-16, authored 2026-08-25) | An evaluation-mode request with a synthetic packet, run in the same session as unrelated live Jira/Confluence context from a different task. | The agent generates only from the supplied synthetic evaluation packet and configured evaluation fixtures; it does not mix in the unrelated live context unless the evaluation prompt explicitly requests retrieval, and if it does, every retrieved source is listed in the response. | Not Run |
...
| QATCA-EVAL-013 | Default ArtifactPack | In production mode with a Jira epic/story link and gathered context, generate QA artifacts without naming a specific artifact. | Defaults to all nine artifacts or batches them in governed order if too large. | Not Run |
| QATCA-EVAL-014 | Single artifact exception | In production mode with a Jira epic/story link and gathered context, generate only CoverageMap. | Returns CoverageMap plus minimal validation notes, ConflictLog, and TBDLog only if applicable. | Not Run |
...
| QATCA-EVAL-016 | Production conflict (tightened 2026-08-26, F9/R-2) | Gathered production context (epic/story, issue links, Confluence search) contains conflicting source evidence for one AC, plus a second, unrelated AC with no conflict. | Does not resolve the conflict and does not generate a test case for either conflicting value; generates the test case(s) for the unrelated AC rather than dropping the entire request; logs the conflict in ConflictLog for human review. | Not Run |
...
| QATCA-EVAL-033 | CSV structural integrity (2026-07-16, R-4; column/blank counts corrected 2026-08-25, re-corrected 2026-08-27) | Stories whose Description/Data values contain commas, plus a multi-step test case. | Every emitted CSV row parses to exactly 15 fields; comma-bearing fields are double-quoted (embedded quotes doubled); continuation rows contain exactly 13 leading blanks then Action and Expected Result; Action is populated whenever Expected Result is; the Story ID appears on each test case's first row; `Priority` is present and populated (default `Low`). | **Partial as of 2026-07-21** (pre-correction; re-run needed against the current 15-column contract) — the dedicated prompts passed cleanly at the time (full quoting incl. doubled embedded quotes; the then-current 13-blank continuation rows), but three organic rows in the same run still violated field counts (unquoted commas; one 14-field metadata row). See the [v0.25 results review](../reports/qa-test-case-architect-v2-studio-evaluation-v0.25-results-2026-07-21.md); numbers there predate both the 2026-08-25 14-column correction and the 2026-08-27 15-column re-correction (Change Log v0.34). |
| QATCA-EVAL-037 | Product/state per-combination default (2026-08-25 decision, §2) | A story naming two distinct product/state combinations, e.g., `NJ HO3` and `AZ HO3 with no fungal endorsement`. | Generates a separate test per combination by default (at least two distinct tests), not one generalized test covering both. | Not Run |
...
| QATCA-EVAL-041 | Confirm-before-generate columns (2026-08-25 decision, §5; column count corrected 2026-08-27) | User requests XRAY CSV for a story with no mention of column requirements. | Before generating, the agent confirms with the user that the standard 15 columns are sufficient rather than generating silently. | Not Run |
| QATCA-EVAL-042 | Additional-column request handling (2026-08-25 decision, §5; column count corrected 2026-08-27) | User explicitly asks the agent to add a column not in the shared 15-column contract (e.g., a team-specific "Risk Tier" column). | The agent does not add the column on the spot: it generates the standard 15-column output, logs the requested addition in TBDLog as a proposed team-overlay change, and tells the user a new column requires a governed overlay decision rather than an ad hoc addition. | Not Run |
| QATCA-EVAL-043 | Priority always populated (2026-08-27 decision, v0.34) | Generate XRAY CSV for a story with no priority signal in the source evidence. | Every row's `Priority` column is populated with `Low` by default; the column is never omitted and never left blank, since Priority is required to close the underlying Jira ticket downstream. | Not Run |
