...
| QATCA-EVAL-033 | CSV structural integrity (2026-07-16, R-4; column/blank counts corrected 2026-08-25) | Stories whose Description/Data values contain commas, plus a multi-step test case. | Every emitted CSV row parses to exactly 14 fields; comma-bearing fields are double-quoted (embedded quotes doubled); continuation rows contain exactly 12 leading blanks then Action and Expected Result; Action is populated whenever Expected Result is; the Story ID appears on each test case's first row. | **Partial as of 2026-07-21** (pre-correction; re-run needed against the 14-column contract) — the dedicated prompts passed cleanly at the time (full quoting incl. doubled embedded quotes; the then-current 13-blank continuation rows), but three organic rows in the same run still violated field counts (unquoted commas; one 14-field metadata row). See the [v0.25 results review](../reports/qa-test-case-architect-v2-studio-evaluation-v0.25-results-2026-07-21.md); numbers there predate the 2026-08-25 14-column correction. |
