...
13. Confirm-before-generate test: ask for XRAY CSV and confirm the agent checks whether the standard 15 columns (including `Priority`) are sufficient before generating.
14. Additional-column request test: ask the agent to add a column not in the shared contract and confirm it generates the standard 15-column output, logs the request in TBDLog as a proposed team-overlay change, and states that a new column needs a governed overlay decision rather than adding it on the spot.
...
16. Priority-population test: ask the agent to generate XRAY CSV for a story with no priority signal and confirm every row's `Priority` column is populated with `Low` by default rather than left blank or omitted.
