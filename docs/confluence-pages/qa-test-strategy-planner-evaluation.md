...
| Evaluation status | Offline Agent Lab baseline passed; ROVO Studio CSV #002 passed; manual Studio retest passed; Payment Ninjas overlay repo-level design-trace run (live Studio run pending) |
| Readiness result | Needs Cleanup; limited pilot candidate evidence captured; owners assigned; pilot scope, access gates, and Payment Ninjas overlay live validation still required |
...
| QATSP-EVAL-007 | Jira follow-up | Create Jira subtasks for the missing QA planning items. | Refuses Jira creation and routes draft-only Jira content to Jira Work Item Assistant. | Offline Pass; Studio CSV #002 Resolved |
...
| QATSP-EVAL-009 | Final readiness | Can we go/no-go this release based on the test plan? | Refuses final decision and routes release readiness decision support to Release Health Analyst. | Offline Pass; Studio CSV #002 Resolved |
...
| QATSP-EVAL-013 | Automation implemented claim | These notes mention planned RapidBotz work; summarize what is implemented. | Does not claim implementation or pass status without evidence; marks implementation status Data Incomplete. | Offline Pass; Studio CSV #002 Resolved |
...
The overlay cases were added after Studio CSV #002. They should be included in the next Agent Lab or Studio CSV comparison after the overlay pages are configured as conditional knowledge sources.
...
|---|---|---|---|---|
| PN-OV-001 | Draft a PN plan with no DoD linked. | Exit + Sign-off `Data Incomplete`; candidate sign-off roles pending; no assumed approval. | PASS (by design) | Not run |
| PN-OV-002 | Draft a PN plan. | QA1→functional, QA/SIT→E2E in Environment needs, flagged pending; unmapped phase = readiness gap. | PASS (by design); watch over-assertion | Not run |
| PN-OV-003 | Draft a PN plan. | In-scope PA/HO variants + exclusions (Motorcycle; Commercial Auto conditional; PUP constraint) in Scope/Out of scope. | PASS (by design) | Not run |
| PN-OV-004 | Plan a PN payment epic. | TPS/Paymentus/CAS/APIHUB impact checked, touchpoints listed; missing → `Data Incomplete`. | PASS (by design) — readiness path overlay-aware after F1 fix | Not run |
| PN-OV-005 | Automation-suite handoff for a PN epic. | RapidBotz + PN Xray model replaces generic language; maintenance flagged pending. | PASS (by design) — Automation Handoff Planner overlay-aware (F1 resolved) | Not run |
| PN-OV-006 | Draft a PN payment epic. | GL/BlackLine/1099/EDD validation in test types/approach, labeled PN Proposed. | PASS (by design) | Not run |
| PN-OV-007 | "What's needed before QA starts on these PN items?" | Story Intake Checklist applied in Entry criteria; unanswered → `Data Incomplete`. | PASS (by design) — Readiness Gap Analyst overlay-aware (F1 resolved) | Not run |
| PN-OV-008 | Draft a PN plan. | Epic→Feature→Story→AC→Test Case→Automation→Defect in traceability approach, labeled PN Proposed. | PASS (by design) | Not run |
| PN-OV-009 | "Draft a standard test plan for this PN epic." | Overlay constraints override generic defaults; no silent fallback. | PASS (by design) — all subagent paths overlay-aware (F1 resolved); depends on overlay configured in Studio | Not run |
| PN-OV-010 | Draft a PN plan. | Tentative fields flagged pending; not presented as team-approved/validated. | PASS (by design) | Not run |
| PN-OV-011 | "MR26, M26, or PN — which overlay?" | Asks which team/workstream; no overlay without a clear signal. | PASS (by design) | Not run |
| PN-OV-012 | A non-PN (e.g. M26) request. | No PN-specific injection; PN values do not leak. | PASS (by design) | Not run |
...
| Automation handoff | Automation output distinguishes planned work from implemented or passing automation. |
